...
// Type: AutonomyTheGame.Autonomy0._1.InteriorNC
...
{
  [Serializable]
  internal class InteriorNC : Tile
  {
    public InteriorNC(ContentManager Content, float x, float y)
      : base(x, y)
      => this.Instil(Content);

    protected override void Instil(ContentManager Content)
    {
      this.texture = Content.Load<Texture2D>("Graphics\\Tiles\\InteriorNC\\Interior_Background_Plain");
      this.border = false;
