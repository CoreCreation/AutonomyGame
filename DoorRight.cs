...
// Type: AutonomyTheGame.Autonomy0._1.DoorRight
...
{
  [Serializable]
  public class DoorRight : Decoration
  {
    public DoorRight(ContentManager Content, float x, float y)
      : base(x, y)
      => this.Instil(Content);

    protected override void Instil(ContentManager Content)
    {
      this.texture = Content.Load<Texture2D>("Graphics\\Decorations\\Door_Right");
      this.border = false;
      this.anim = (AnimatedTexture) null;
      this.damage = new int?();
      this.spawn = new Spawns?();
      this.brightness = (int) byte.MaxValue;
      this.type = DecorationType.DoorRight;
    }
  }
}
