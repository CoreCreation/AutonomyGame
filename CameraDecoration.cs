...
// Type: AutonomyTheGame.Autonomy0._1.CameraDecoration
...
{
  [Serializable]
  public class CameraDecoration : Decoration
  {
    public CameraDecoration(ContentManager Content, float x, float y)
      : base(x, y)
      => this.Instil(Content);

    protected override void Instil(ContentManager Content)
    {
      this.texture = Content.Load<Texture2D>("Graphics\\Decorations\\SecurityCam");
      this.border = false;
      this.anim = (AnimatedTexture) null;
      this.damage = new int?();
      this.spawn = new Spawns?();
      this.brightness = (int) byte.MaxValue;
      this.type = DecorationType.Camera;
    }
  }
}
