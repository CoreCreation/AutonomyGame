...
// Type: AutonomyTheGame.Autonomy0._1.PlatformRight
...
{
  [Serializable]
  internal class PlatformRight : Tile
  {
    public PlatformRight(ContentManager Content, float x, float y)
      : base(x, y)
      => this.Instil(Content);
    ...
    protected override void Instil(ContentManager Content)
    {
      ...
      this.texture = Content.Load<Texture2D>("Graphics\\Tiles\\Platforms\\Platform");
      this.collision = true;
      this.damage = new int?();
      this.speedMod = new float?();
      this.spawn = new Spawns?();
      this.type = TileType.PlatformRight;
