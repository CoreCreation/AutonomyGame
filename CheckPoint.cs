...
// Type: AutonomyTheGame.Autonomy0._1.CheckPoint
...
{
  [Serializable]
  internal class CheckPoint : Tile
  {
    public CheckPoint(ContentManager Content, float x, float y)
      : base(x, y)
      => this.Instil(Content);
    ...
    protected override void Instil(ContentManager Content)
    {
      ...
      this.type = TileType.CheckPoint;
      this.brightness = (int) byte.MaxValue;
      this.absorbValue = 0.0;
      this.activationTile = true;
    }

    public override void Activate(Game1 game, Entity activator) => game.map1.SetSpawn((Tile) this);
  }
}
